...
//	SetRegistryKey(_T("Local AppWizard-Generated Applications"));
    CString strApp = _T("Moosburger\\Sysview95");
    m_regManager.Open(strApp, 30, TRUE);
    ...
    if (GetModuleFileName(NULL, cBufDir, sizeof(cBufDir)) != 0)
    {
        ...
        strSubKey = _T("");
        strValue = _T("APP_PATH");
        if (m_riAppPath.Connect(&m_regManager, strSubKey, 
            strValue, bCreated))
        {
        ...
        strValue = _T("SYSTEM_INTEGRATION");
        if (m_ridIntegration.Connect(&m_regManager, strSubKey, 
            strValue, bCreated))
        {
