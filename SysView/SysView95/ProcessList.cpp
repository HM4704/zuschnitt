...
BOOL CProcessList::SetupRegistryValues()
{
    ...
    if (pApp)
    {
        ...
        m_dwCntColumns++;
        strSubKey = _T("Prozesse");
        strValue = _T("DETAILS");
        if (m_riFieldsToShow.Connect(pApp->GetRegistryManager(), 
            strSubKey, strValue, bCreated) == TRUE)
        {
            if (bCreated == FALSE)
            {
...
BOOL CProcessList::UpdateList()
{
	...
	for (int iItem = 0; iItem < (int)m_dwExistProcs; iItem++)  // insert the items and subitems into the list view.
    {
        if (iItem < iCount)
        {
            ...
            if (TRUE)  //?? 0 bei WinXP Idle-Prozess!!!
            {
                    ...
                    PROCESSENTRY32 *pPE, CPU_USAGE* pCuBuf, DWORD dwEntries)
{
    ...
    {
        // copy only file-name
        char* pFileName = strrchr(pPE[i].szExeFile, '\\');
        if (pFileName == NULL)
        {
            pFileName = pPE[i].szExeFile;
        }
        else
        {
            pFileName++;
...
void CProcessList::OnProcMemusage() 
{
    ...
    {
        MessageBox("Diese Funktion wird zur Zeit nicht unterstützt!", "Achtung");
    }
}
