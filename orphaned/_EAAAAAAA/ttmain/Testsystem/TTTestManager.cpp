...
BOOL CTTTestManager::Initialize(CFrameWnd* pFrmWnd, CRegistryManager* pRegMan)
{
    ...
    m_Config.strBaseTestDir = "Testsystem\\Testdateien\\";    
...
BOOL CTTTestManager::OpenLogFile(void)
{
    ...
    m_osLogFile << "Zuschnitt-Version:      1.5\n";
    ...
    while (m_isTestFile.eof() == false)
    {
        ...
        if ((str.IsEmpty() == FALSE) && (str.GetAt(0) == '['))
        {
            ...
            while (m_isTestFile.eof() == false)
            {
                ...
                // Parameter einlesen
                if (sscanf(str, "%s %d %d %d %d %d %d %d %d %d", strTTyp,&iFuellung,&iAnz,&iOkf,&iBreite,
                    &iHoehe,&iObtHoehe,&iBogen,&iSehnenH,&iProfilD) !=
                    10)
                    // falsches Format
                    return FALSE;
                else
            ...
            {
                // "F:" suchen
                int iGeteilt, iOberteil, iFenster, iGlasart, iBreite;
                ...
                // Parameter einlesen
                if (sscanf(str, "%d %d %d %d %d", &iGeteilt,&iOberteil,&iFenster,
                    &iGlasart,&iBreite) !=
                    5)
                    // falsches Format
                    return FALSE;
...
BOOL CTTTestManager::CompareSet(CTorDoor* pRefTor, CTorDoor* pTor, char* strRefFile)
{
    ...
    if (pRefTor->HolzElemente->GetSize() != 0)
    {
        for (int i = 0; i < pRefTor->HolzElemente->GetSize(); i++)
        {
    ...
    if (pRefTor->BetoPlanElemente->GetSize() != 0)
    {
        for (int i = 0; i < pRefTor->BetoPlanElemente->GetSize(); i++)
        {
            CHolzElement* pRefElem = (CHolzElement*)pRefTor->HolzElemente->GetAt(i);
            CHolzElement* pElem    = (CHolzElement*)pTor->HolzElemente->GetAt(i);
            ASSERT(pRefElem);
...
#define DL  ","
    ...
    m_osLogFile <<pTor->Size.ObtHoehe<<DL<<pTor->bBogen<<NL;
...
void CTTTestManager::DumpFluegel(CFlParam* pFl)
{
    ...
    m_osLogFile << pFl->GArt<<DL<<pFl->iBreite<<NL;
}
...
void CTTTestManager::DumpProfil(CProfileElem* pE, char* strName)
{
    ...
    m_osLogFile <<pE->iWinkel1<<DL<<pE->iWinkel2<<NL;
}
