...
void CTorDoor::getAddValues(int count, int &iAddQuerT, int &iAddSenkT, int &iAddOberT, int &iAddUnterT)
{
  if (Typ != 4)
  {
...
void CTorDoor::computeProfile(CFlParam* pFl, int count)
{
  ...
  getAddValues(count, addQuerT, addSenkT, addOberT, addUnterT);
...
//    insertProfil(1*StueckZahl, querProf, WAAGRECHT, HOLZAUF);
    ...
    int senkProf = 10*Size.ObtHoehe + 10;   //????
    tempQuerProf -= 115;
    insertProfil(2*StueckZahl, senkProf, SENKRECHT, HOLZUNTEN);
  ...
  {
    int tempQuerProf = querProf;
    int senkProf = 10*Size.ObtHoehe - 35;
    tempQuerProf = (tempQuerProf - 206)/2;
