...
CSchiebeTuer::CSchiebeTuer() : CTuerN()
{ 
    m_rollAppBmp.LoadBitmap(IDB_ROLLAPP2); 
}

CSchiebeTuer::CSchiebeTuer(CTorDoor* ct) : CTuerN(ct) 
{
    m_rollAppBmp.LoadBitmap(IDB_ROLLAPP2); 
}
...
void CSchiebeTuer::drawTT(HDC hdc, int x, int y, int b, int h, HFONT fntSmall)
{
   ...
   ::StretchBlt(
      hdc,      // handle to destination DC
      x + Size.ZBreite - (0.3f*bm.bmWidth), // x-coord of destination upper-left corner
      -1*y  + aSp, // y-coord of destination upper-left corner
      scF*0.1f*bm.bmWidth,   // width of destination rectangle
      scF*0.1f*bm.bmHeight,  // height of destination rectangle
      m_rollAppBmp.GetCompatibleDC(hdc),       // handle to source DC
    ...
    // sonst alles wie Tuer
    CTuerN::drawTT(hdc, x, y, b, h, fntSmall);
}
