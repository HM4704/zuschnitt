...
BEGIN_MESSAGE_MAP(CStaticLink, CStatic)
	...
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
    ON_MESSAGE(WM_LINK_CLICKED, OnLinkClicked)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CStaticLink::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
    ...
    if (pApp)
    {
        m_hCursLink=pApp->LoadCursor(IDC_TT_HAND);
    }
...
LRESULT CStaticLink::OnLinkClicked(WPARAM wParam, LPARAM lParam)
{
    if (m_link.IsEmpty())         // if URL/filename not set..
        GetWindowText(m_link);    // ..get it from window text
