...
void CFTor::drawTT(HDC hdc, int x, int y, int b, int h)
{
...
*/
   ...
   for (i=0; i<count; i++)
   {
	 ...
	 if (FluegelP->FArt == SF)
	 {
	   MoveToEx(hdc, x+i*Fluegel+Fluegel/2, -(y+ scF*(Size.ObtHoehe/2)), NULL);
	   LineTo(hdc, x+i*Fluegel+Fluegel/2, -(y+ scF*(Size.ObtHoehe+40)));
	   int xy =  scF*SFPFXY;
	   LineTo(hdc, x+i*Fluegel+Fluegel/2-xy, -(y+ scF*(Size.ObtHoehe+40)-xy));
	   MoveToEx(hdc, x+i*Fluegel+Fluegel/2, -(y+ scF*(Size.ObtHoehe+40)), NULL);
	   LineTo(hdc, x+i*Fluegel+Fluegel/2+xy, -(y+ scF*(Size.ObtHoehe+40)-xy));
	 }
	 else
