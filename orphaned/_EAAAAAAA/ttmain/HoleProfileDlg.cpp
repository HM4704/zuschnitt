...
CHoleProfileDlg::CHoleProfileDlg(CPtrArray* pContainer, HP_OPT_SETTINGS* pSetsOpt, CWnd* pParent /*=NULL*/)
	: CDialog(CHoleProfileDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CHoleProfileDlg)
	m_iOptTime = 0;
	m_bDoProfileOpt = FALSE;
	//}}AFX_DATA_INIT
...
void CHoleProfileDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_BH_PROFIL_OPT_TIME, m_iOptTime);
	DDX_Check(pDX, IDC_BH_PROFIL_OPT, m_bDoProfileOpt);
	//}}AFX_DATA_MAP
}
...
BOOL CHoleProfileDlg::OnInitDialog() 
{
    ...
    CopyOptSettings(TRUE);

	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}

void CHoleProfileDlg::CopyOptSettings(BOOL bLoad)
{
    if (bLoad == TRUE)
    {
        // in Dialog-Elemente laden
        m_bDoProfileOpt = m_pSetsOpt->bDoOpt;
        m_iOptTime      = m_pSetsOpt->iOptTime;
    ...
    else
    {
        ...
        m_pSetsOpt->bDoOpt = m_bDoProfileOpt;
        m_pSetsOpt->iOptTime = m_iOptTime;
    }
}
