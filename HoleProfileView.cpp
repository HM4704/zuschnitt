...
void CHoleProfileView::OnDraw(CDC* pDC)
{
    ...
    if (pDC->IsPrinting())
    {
        ...
        m_iPageLen = (int)(10*MM_PER_INCH*pDC->GetDeviceCaps(HORZRES)/iDpiHorz) - m_iLeftX - 20;
        m_iPageHeight = (int)(10*MM_PER_INCH*pDC->GetDeviceCaps(VERTRES)/iDpiVert) - m_iTopY -20;
    }
    else
    {
...
//    pDC->SetMapMode(MM_LOMETRIC);
    ...
    CPen* orgPen = (CPen*)pDC->SelectObject(&m_bPen);
    DrawHeader(pDC);
    DrawGrid(pDC);
    DrawData(pDC);
...
void CHoleProfileView::DrawHeader(CDC* pDC)
{
  ...
  pDC->Rectangle(m_iLeftX, -(m_iTopY-rowH-size.cy-10), 
      m_iLeftX+m_iPageLen-1, -(m_iTopY-rowH));
  ...
  strcpy(buf, "Zuschnitt für RP Profile");
...
//  pDC->LineTo((m_iLeftX+m_iPageLen), -(m_iTopY-10));
  ...
  pDC->MoveTo(actX + 550, -(m_iTopY - rowH));
  pDC->LineTo(actX + 550, actY);
...
void CHoleProfileView::PrintCutList(CDC* pDC, CPtrArray* pList, int& iY, int iX, int iWidth)
{
    char szTemp[2000];

    szTemp[0] = 0;
    for (int c = 0; c < pList->GetSize(); c++)
    {
        char szCut[100];
        CProfileCutElem* pCut = (CProfileCutElem*)pList->GetAt(c);
        if (pCut)
        {
            ...
            if (pCut->m_eAction == caLength)
            {
                sprintf(szCut, "[%ld], ", pCut->m_dwLength);
                strcat(szTemp, szCut);
            }
        }
    }
    ...
    //?? pDC->LPtoDP(&size);
    int iAvCharCnt = iWidth/size.cx - 7;
    ...
    while (TRUE)
    {
        ...
        ::TextOut(pDC->GetSafeHdc(), iX, -iY, pOut, strlen(pOut));
        iY += (rowH/4)*3;
    ...
    for (int i = 0; i < pDoc->m_Profiles.GetSize(); i++)
    {
        ...
        if (pPrf)
        {
            ...
            if (p)
            {
                ...
                PrintCutList(pDC, p, iActY, iActX, iSpaceCutList);
            }
            // --> Schnittliste Ende
            pDC->MoveTo(m_iLeftX, -(iActY + 10));
            pDC->LineTo(m_iLeftX+m_iPageLen-1, -(iActY + 10));
            iActY += rowH/2;
        }
    }
    if (pDoc->m_CutListOpt.GetSize() > 0)
    {
        // optimierte Schnittliste mit ausgeben
        ::SelectObject(pDC->GetSafeHdc(), m_boldFont);
        strcpy(szTemp, "Optimierte Schnittliste:");
        ::TextOut(pDC->GetSafeHdc(), iActX, -iActY, szTemp, strlen(szTemp));
        ::SelectObject(pDC->GetSafeHdc(), m_stdFont);
        iActY += rowH;
        PrintCutList(pDC, &pDoc->m_CutListOpt, iActY, iActX, iSpaceCutList);
    }
}
